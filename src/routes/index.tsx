...
const heroImage = "/images/hero-certificate-distribution.jpg";
      ...
      <section className="relative overflow-hidden bg-[#f7f8f9] pt-4 pb-10 md:pt-6 md:pb-16">
        ...
        <div className="container-page relative z-10 grid items-center gap-8 md:grid-cols-2">
          ...
          <ScrollReveal direction="right" delay={0.1} scale className="max-w-xl xl:pr-10">
            <div className="inline-flex items-center gap-2.5 rounded-full bg-gradient-to-r from-[#0eb39e]/15 to-[#0eb39e]/25 border-2 border-[#0eb39e]/50 px-4 sm:px-5 py-2 mb-5 shadow-md backdrop-blur-md">
              <span className="relative flex h-3 w-3 shrink-0">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#0eb39e] opacity-75"></span>
                <span className="relative inline-flex rounded-full h-3 w-3 bg-[#0eb39e]"></span>
              </span>
              <p className="text-sm sm:text-base md:text-lg font-black uppercase tracking-wider text-[#09796b]">
                Welcome to Kakatheeya Foundation
              </p>
            </div>
          ...
          <ScrollReveal direction="left" delay={0.2} className="relative mx-auto w-full max-w-[420px] group/hero">
            ...
            <div className="relative z-10 pt-4">
              ...
              <img
                src={heroImage}
                alt="Kakatheeya Foundation & Association certificate distribution event"
              ...
              <div className="absolute -bottom-6 -left-16 z-20 w-[310px] rounded-xl bg-white p-5 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.15)] hidden sm:block transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_50px_-15px_rgba(0,0,0,0.2)] border border-slate-100">
                ...
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <div className="leading-tight text-right pr-2">
                    <span className="block font-black text-[#f97316] text-[16px]">More than 2000+</span>
                    <span className="text-xs text-slate-700 font-bold block">Students Placed</span>
                    <span className="text-[11px] text-emerald-600 font-bold block mt-0.5">20+ Companies</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <Section title="Our programmes" description="Two tracks, both built around getting a real job at the end." className="max-w-6xl mx-auto px-4 md:px-8 !py-4 md:!py-6">
          <div className="grid gap-6 lg:grid-cols-2 lg:gap-8">
            <ScrollReveal direction="right" delay={0.1}>
              <Card className="flex h-full flex-col overflow-hidden rounded-2xl border-2 border-[#8b1a1a]/30 bg-card shadow-md ring-1 ring-black/5 transition-all duration-300 hover:-translate-y-1.5 hover:border-[#8b1a1a] hover:shadow-2xl hover:shadow-[#8b1a1a]/15">
                <div className="h-1.5 w-full bg-gradient-to-r from-[#8b1a1a] via-[#e8a040] to-[#0eb39e]" />
                <img
                ...
                <CardContent className="flex flex-1 flex-col p-4 sm:p-5">
                  <div>
                    <div className="inline-block rounded-full bg-[#8b1a1a]/10 px-2.5 py-0.5 border border-[#8b1a1a]/20 mb-2">
                      <p className="text-[10px] font-bold uppercase tracking-wide text-[#8b1a1a]">35 days · 100% Free</p>
                    </div>
                    <h3 className="mt-1 font-display text-lg font-bold text-foreground">{vocational.title}</h3>
                    <ul className="mt-3 grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs text-muted-foreground">
                      {["Tally", "GST", "Spoken English", "Computer basics", "Life skills", "Confidence building"].map(
                        (c) => (
                          <li key={c} className="flex items-center gap-1.5 font-medium">
                            <BadgeCheck className="h-3.5 w-3.5 text-primary shrink-0" /> {c}
                          </li>
                        ),
                      )}
                    </ul>
            ...
            <ScrollReveal direction="left" delay={0.2}>
              <Card className="flex h-full flex-col overflow-hidden rounded-2xl border-2 border-[#8b1a1a]/30 bg-card shadow-md ring-1 ring-black/5 transition-all duration-300 hover:-translate-y-1.5 hover:border-[#8b1a1a] hover:shadow-2xl hover:shadow-[#8b1a1a]/15">
                <div className="h-1.5 w-full bg-gradient-to-r from-[#0eb39e] via-[#e8a040] to-[#8b1a1a]" />
                <img
                ...
                <CardContent className="flex flex-1 flex-col p-4 sm:p-5">
                  <div>
                    <div className="inline-block rounded-full bg-[#0eb39e]/10 px-2.5 py-0.5 border border-[#0eb39e]/20 mb-2">
                      <p className="text-[10px] font-bold uppercase tracking-wide text-[#0b8575]">6 months · Diploma / B.Tech</p>
                    </div>
                    <h3 className="mt-1 font-display text-lg font-bold text-foreground">Industrial &amp; Practical Training</h3>
                      ...
                      {industrial.map((p) => (
                        <li key={p.slug}>
                          <Link
                            to="/programs/$slug"
                            params={{ slug: p.slug }}
                            className="inline-flex rounded-md border border-border/80 bg-secondary/70 px-2.5 py-1 text-[11px] font-medium transition-colors hover:bg-primary hover:text-white"
                          >
                            {p.title.split("— ")[1]}
                          </Link>
                        </li>
                  ...
                  <div className="mt-auto pt-4 border-t border-border/60">
                    <Button asChild size="sm" variant="outline" className="font-semibold shadow-sm">
